...
constexpr static int MAP_W = 160;
constexpr static int MAP_H = 120;
constexpr static int BLOCK_SIZE = 8;
constexpr static int BOLD_BORDER_SIZE = 2;
constexpr static int THIN_BORDER_SIZE = 1;
...
struct Nation
{
	...
	Nation() {}
	Nation(uint8_t R, uint8_t G, uint8_t B) : r(R), g(G), b(B) {
...
struct Block {
	...
	float GetAvailableSpace() { return std::max({ powf((float)building_level, 5.2f) - food - product - man_level, 0.f }); }
	float GetThroughOut() const { return geo_desc->mho * powf(road_level * 1.f / ROAD_LEVEL_MAX, 2); }
};
